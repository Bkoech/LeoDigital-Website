<?php

use Illuminate\Database\Seeder;
use Illuminate\Database\Eloquent\Model;

class PortfolioTableSeeder extends Seeder
{
    /**
     * Run the database seeds.
     *
     * @return void
     */
    public function run()
    {
        DB::table('portfoliocategorys')->insert([
            'name' => 'Web',
            'slug' => 'web',
            'created_at' => \Carbon\Carbon::now(),
            'updated_at' => \Carbon\Carbon::now(),
        ]);
        DB::table('portfoliocategorys')->insert([
            'name' => 'SEO',
            'slug' => 'seo',
            'created_at' => \Carbon\Carbon::now(),
            'updated_at' => \Carbon\Carbon::now(),
        ]);
        DB::table('portfoliocategorys')->insert([
            'name' => 'SMM',
            'slug' => 'smm',
            'created_at' => \Carbon\Carbon::now(),
            'updated_at' => \Carbon\Carbon::now(),
        ]);
        DB::table('portfoliocategorys')->insert([
            'name' => 'PPC',
            'slug' => 'ppc',
            'created_at' => \Carbon\Carbon::now(),
            'updated_at' => \Carbon\Carbon::now(),
        ]);

        DB::table('portfolios')->insert([
            'title' => '{"ru":"Тестовое название 1"}',
            'slug' => 'test-name-1',
            'description' => '{"ru":"Тестовое описание"}',
            'content' => '{"ru":"Тестовые контент"}',
            'options' => '{"ru":[{"name":"Проект тестовый","desc":"www.site.com"},{"name":"Услуга","desc":"SEO SMM Develop"},{"name":"Срок сотрудничества","desc":"1 год"}]}',
            'image' => 'http://via.placeholder.com/600x400',
            'status' => 'PUBLISHED',
            'created_at' => \Carbon\Carbon::now(),
            'updated_at' => \Carbon\Carbon::now(),
        ]);
        DB::table('portfolios')->insert([
            'title' => '{"ru":"Тестовое название 2"}',
            'slug' => 'test-name-2',
            'description' => '{"ru":"Тестовое описание"}',
            'content' => '{"ru":"Тестовые контент"}',
            'options' => '{"ru":[{"name":"Проект тестовый","desc":"www.site.com"},{"name":"Услуга","desc":"SEO SMM Develop"},{"name":"Срок сотрудничества","desc":"1 год"}]}',
            'image' => 'http://via.placeholder.com/600x400',
            'status' => 'PUBLISHED',
            'created_at' => \Carbon\Carbon::now(),
            'updated_at' => \Carbon\Carbon::now(),
        ]);
        DB::table('portfolios')->insert([
            'title' => '{"ru":"Тестовое название 3"}',
            'slug' => 'test-name-3',
            'description' => '{"ru":"Тестовое описание"}',
            'content' => '{"ru":"Тестовые контент"}',
            'options' => '{"ru":[{"name":"Проект тестовый","desc":"www.site.com"},{"name":"Услуга","desc":"SEO SMM Develop"},{"name":"Срок сотрудничества","desc":"1 год"}]}',
            'image' => 'http://via.placeholder.com/600x400',
            'status' => 'PUBLISHED',
            'created_at' => \Carbon\Carbon::now(),
            'updated_at' => \Carbon\Carbon::now(),
        ]);
        DB::table('portfolios')->insert([
            'title' => '{"ru":"Тестовое название 4"}',
            'slug' => 'test-name-4',
            'description' => '{"ru":"Тестовое описание"}',
            'content' => '{"ru":"Тестовые контент"}',
            'options' => '{"ru":[{"name":"Проект тестовый","desc":"www.site.com"},{"name":"Услуга","desc":"SEO SMM Develop"},{"name":"Срок сотрудничества","desc":"1 год"}]}',
            'image' => 'http://via.placeholder.com/600x400',
            'status' => 'PUBLISHED',
            'created_at' => \Carbon\Carbon::now(),
            'updated_at' => \Carbon\Carbon::now(),
        ]);
        DB::table('portfolios')->insert([
            'title' => '{"ru":"Тестовое название 5"}',
            'slug' => 'test-name-5',
            'description' => '{"ru":"Тестовое описание"}',
            'content' => '{"ru":"Тестовые контент"}',
            'options' => '{"ru":[{"name":"Проект тестовый","desc":"www.site.com"},{"name":"Услуга","desc":"SEO SMM Develop"},{"name":"Срок сотрудничества","desc":"1 год"}]}',
            'image' => 'http://via.placeholder.com/600x400',
            'status' => 'PUBLISHED',
            'created_at' => \Carbon\Carbon::now(),
            'updated_at' => \Carbon\Carbon::now(),
        ]);

        DB::table('portfolio_portfoliocategory')->insert([
            'portfolio_id' => '1',
            'portfolio_category_id' => '1',
            'created_at' => \Carbon\Carbon::now(),
            'updated_at' => \Carbon\Carbon::now(),
        ]);
        DB::table('portfolio_portfoliocategory')->insert([
            'portfolio_id' => '2',
            'portfolio_category_id' => '2',
            'created_at' => \Carbon\Carbon::now(),
            'updated_at' => \Carbon\Carbon::now(),
        ]);
        DB::table('portfolio_portfoliocategory')->insert([
            'portfolio_id' => '3',
            'portfolio_category_id' => '4',
            'created_at' => \Carbon\Carbon::now(),
            'updated_at' => \Carbon\Carbon::now(),
        ]);
        DB::table('portfolio_portfoliocategory')->insert([
            'portfolio_id' => '4',
            'portfolio_category_id' => '4',
            'created_at' => \Carbon\Carbon::now(),
            'updated_at' => \Carbon\Carbon::now(),
        ]);
        DB::table('portfolio_portfoliocategory')->insert([
            'portfolio_id' => '1',
            'portfolio_category_id' => '1',
            'created_at' => \Carbon\Carbon::now(),
            'updated_at' => \Carbon\Carbon::now(),
        ]);
    }
}
